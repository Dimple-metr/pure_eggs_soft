...
function opening_stock_value_details($dbcon,$start_date){
    ...
    $str.='
            <tr>
                    <td>Stock</td>
                    <td style="text-align: right;">'.number_format((float)$opning_balance, 2).'</td>
            </tr>
            ';
    $str.="</table>"; 
    return $str;
...
function get_direct_expance($dbcon, $where_date){
...
function get_opening_stock($dbcon, $start_date, $end_date){
        $inventory_management = $dbcon->query("SELECT inventory_management FROM tbl_company as comp WHERE company_id=".$_SESSION['company_id'])
                ->fetch_object()->inventory_management;
        ...
        if($inventory_management){
            ...
            $purchase = get_purchase_account($dbcon, $start_date, $end_date);
            $sales = get_sales_account($dbcon, $start_date, $end_date);
            $opening_balance = round($purchase['value'] - $sales['sales_value']);
        }
        ...
        $str.='
                <tr>
                        <td>Stock</td>
                        <td style="text-align: right;">'.number_format((float)$opening_balance, 2).'</td>
                </tr>
                ';
        $str.="</table>"; 
            
        $opening_stock['entries'] = $str;
        $opening_stock['value'] = $opening_balance;
