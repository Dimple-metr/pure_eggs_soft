...
{ 
    //if(@isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'],DOMAIN) !== false) 
	{
		...
		if(strtolower($POST['mode']) == "fetch") {
			...
			$appData = array();
			$i=1;
			$aColumns = array('receipt_id','receipt_no', 'vender.company_name','payment.payment_mode','cheque_dtl', 'receipt.total_paid_amount','receipt_date','receipt.payment_type','receipt.cdate','pay.chequegenerateid','pay.generat_status','receipt.user_id');
			$sIndexColumn = "receipt_id";
			$isWhere = array("receipt.status = 0".check_user('receipt').$where);
			$sTable = "tbl_receipt as receipt";			
			$isJOIN = array('inner join tbl_customer vender on vender.cust_id=receipt.cust_id','left join tbl_payment_mode payment on payment.paymentmodeid=receipt.payment_mode_id','left join tbl_payment_cheque_generate as pay on pay.purchase_payid=receipt_id and generat_status=0');
			...
			foreach($sqlReturn as $row) {
				...
				$row_data[] = $row['receipt_no'];
				 ...
				 $chedel='';
				 if($row['cheque_dtl']!=""){
					 $chedel="(".$row['cheque_dtl'].")";
				 }
				$row_data[] = $row['payment_mode'].' '.$chedel.'';
				$row_data[] = $row['total_paid_amount'];
				$row_data[] = date('d M, Y',strtotime($row['receipt_date']));
                $btn='';
			...
			*/
			...
			$updatetrancationid2=update_record('tbl_excess', $info,"receipt_id=".$POST['eid'] , $dbcon);
			$updatetrancationid3=update_record('tbl_passbookentry', $info,"trn_id=".$POST['eid']." and trn_table='tbl_receipt'" , $dbcon);	
		...
		else if(strtolower($POST['mode'])== "load_invoiceno")
		{
			...
			$query="select cust.opening_balance,cust.balance_typeid,
					...
					(SELECT sum(g_total) FROM `tbl_proforma_invoice` as perinv where perinv.cust_id=cust.cust_id and perinv.invoice_status!=2) as proinvoice_amount,
					...
					(SELECT sum(g_total) FROM `tbl_credit_note` as credit where credit.cust_id=cust.cust_id and credit.credit_note_status!=2) as credit_amount,
					(SELECT sum(g_total) FROM `tbl_debitnote` as debit where debit.vender_id=cust.cust_id and debit.debit_note_status!=2) as debit_amount,
					...
					$op_balance=0;
						...
						if(!empty($rel['opening_balance']))
						{
							...
							$amount=($op_balance+$rel['paid_amount']+$rel['po_amount']+$rel['cr_excess_amount']+$rel['credit_amount'])-($rel['invoice_amount']+$rel['proinvoice_amount']+$rel['cr_excess_amount']+$rel['purchasepaid_amount']+$rel['debit_amount']);
			...
			union (select "ProInvoice" as type,1 as ref_type,invoice_date as ref_date,invoice_no as ref_no,invoice_id as ref_id,g_total as ref_amount,(select IFNULL(sum(total_amount),0) as qty from  tbl_receipt_trn as trn where status=0 and inv.invoice_id=trn.invoice_id) as pay_amount,inv.cdate  from tbl_proforma_invoice as inv where invoice_status=0 AND cust_id='.$POST['vender_id'].' and inv.g_total>(select IFNULL(sum(total_amount),0) as qty from  tbl_receipt_trn as trn where trn.status=0 and inv.invoice_id=trn.invoice_id))
			...
			union (select "Credit Note" as type,2 as ref_type,credit_note_date as ref_date,credit_note_no as ref_no,credit_note_id as ref_id,g_total as ref_amount,(select IFNULL(sum(total_amount),0) as qty from  tbl_receipt_trn as trn where status=0 and cra.credit_note_id=trn.cradit_note_id) as pay_amount,cra.cdate  from tbl_credit_note as cra where credit_note_status=0 AND cust_id='.$POST['vender_id'].' and cra.g_total>(select IFNULL(sum(total_amount),0) as qty from  tbl_receipt_trn as trn where status=0 and cra.credit_note_id=trn.cradit_note_id))
			
			union (select "Debit Note" as type,1 as ref_type,debitnote_date as ref_date,debitnote_no as ref_no,debitnote_id as ref_id,g_total as ref_amount,(select IFNULL(sum(total_amount),0) as qty from  tbl_receipt_trn as trn where status=0 and deb.debitnote_id=trn.debit_note_id) as pay_amount,deb.cdate  from tbl_debitnote as deb where debit_note_status=0 AND vender_id='.$POST['vender_id'].' and deb.g_total>(select IFNULL(sum(total_amount),0) as qty from  tbl_receipt_trn as trn where status=0 and deb.debitnote_id=trn.debit_note_id))
