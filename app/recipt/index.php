    ...
    //if(@isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'],DOMAIN) !== false) 
	{
		...
		if(strtolower($POST['mode']) == "fetch") {
			...
			if($row['generat_status']=="0" && $row['payment_type']=="2")
				...
				$row_data[] = '
				<a class="btn btn-xs btn-info" data-original-title="Receipt Print" data-toggle="tooltip" data-placement="top" href="'.ROOT.'receipt_sales/'.$row['receipt_id'].'"><i class="fa fa-print"></i></a>
		...
		{
			$query="select * from tbl_invoicetype where status=0 and type_id=8 and company_id=".$_SESSION['company_id'];
