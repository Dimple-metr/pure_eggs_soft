...
{ 
    //if(@isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'],DOMAIN) !== false) 
	{
		...
		if(strtolower($POST['mode']) == "fetch") {
			...
			$appData = array();
			$i=1;
			$aColumns = array('receipt.receipt_id','receipt_no', 'vender.company_name','payment.payment_mode','cheque_dtl', 'receipt.total_paid_amount','receipt_date','receipt.payment_type','receipt.cdate','receipt.user_id');
			...
			$isJOIN = array('inner join tbl_customer vender on vender.cust_id=receipt.cust_id','left join tbl_payment_mode payment on payment.paymentmodeid=receipt.payment_mode_id');
			...
			foreach($sqlReturn as $row) {
				...
				//$row_data[] = "<span ".$col.">".$row['inv']."</span>";
				 $row_data[] = "<span ".$col.">".$row['company_name']."</span>";
				 $chedel='';
				 if($row['cheque_dtl']!=""){
		...
		else if(strtolower($POST['mode'])== "load_invoiceno")
		{
			...
			/*  $query="select cust.opening_balance,cust.balance_typeid,
				...
				$amount=$rel['cr_amount']-$rel1['dr_amount'];
			...
			*/
			...
			echo ' <div class="form-group">
						...
						';
				$query1="select cust.opn_balance as opening_balance,cust.balance_typeid,
							(SELECT sum(inv.paid_amount) FROM `tbl_receipt_trn` as inv 
								left join tbl_receipt as res on res.receipt_id=inv.receipt_id
								where res.cust_id=cust.l_id and inv.status!=2 and res.status!=2 and inv.invoice_id=0 and inv.purchase_id=0 and inv.cradit_note_id=0 and inv.debit_note_id=0 and inv.performa_id=0 and inv.excess_id=0) as paid_amount
				...
				if($due1>0){
					if($rel1['balance_typeid']=="1"){
						 ...
						 echo '<tr id="fieldtr'.$i.'" >
									<td data-label="Type" colspan="3" style="vertical-align:center;'.$colr.'" class="text-center">
									...
									<td data-label="Opening Balance" style="vertical-align:center;'.$colr.'" class="text-center">
										'.$rel1['opening_balance'].'
										<input type="hidden" name="o_ref_amount[]" id="o_ref_amount0" value="'.$rel1['opening_balance'].'" />
									</td>
									<td data-label="" style="vertical-align:center;'.$colr.'" class="text-center">
										'.$due1.'
									...
									<td style="vertical-align:center" class="text-center">
										<input type="number"  title="Enter amount" min="0" onkeyup="paid_total();load_paid_amount(0)" max="'.$due.'" id="o_amount0" name="o_amount[]"  class="form-control" />
										<input type="hidden" name="last_use[]" id="last_use0" value="" />
									</td>
									<td style="vertical-align:center;" class="text-center">
				...
				if(mysqli_num_rows($result)>0)
				{
					...
					while($rel=mysqli_fetch_assoc($result))
					{
						...
						//$due=$rel['pay_amount'];
						 ...
						 echo '<tr id="fieldtr'.$i.'" >
								<td data-label="Type" style="vertical-align:center;'.$colr.'" class="text-center">
									'.$rel['type'].'
								...
								<td data-label="Ref No" style="vertical-align:center;'.$colr.'" class="text-center">
									'.$rel['ref_no'].'
									<input type="hidden" name="o_ref_id[]" id="o_ref_id'.$i.'" value="'.$rel['ref_id'].'" />
								</td>
								
								<td data-label="Date" style="vertical-align:center;'.$colr.'" class="text-center">
								...
								<td style="vertical-align:center" class="text-center">
									<input type="number"  placeholder="Amount" title="Enter amount" min="0" onkeyup="paid_total();load_paid_amount('.$i.')" max="'.$due.'" id="o_amount'.$i.'" name="o_amount[]"   class="form-control" />
									<input type="hidden" name="last_use[]" id="last_use'.$i.'" value="" />
								</td>
								<td style="vertical-align:center;" class="text-center">
