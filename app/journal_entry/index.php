...
if($_POST != NULL) {
	...
	$POST = bulk_filter($dbcon,$_GET);
		...
		$where='';
			...
			$where.="  and journal_date>='".date('Y-m-d',strtotime($s_date[0]))."' AND journal_date<='".date('Y-m-d',strtotime($s_date[1]))."'";
			$appData = array();
			$i=1;
			...
			foreach($sqlReturn as $row) {
				...
				$addpayment='';$delete='';$edit='';
					...
					$edit='<a class="btn btn-xs btn-warning" data-original-title="Edit" data-toggle="tooltip" data-placement="top" href="'.ROOT.'journal_entry_edit/'.$row['journal_id'].'"><i class="fa fa-pencil"></i></a>';
			...
			$query_invoicetype = $dbcon->query("UPDATE tbl_invoicetype SET taxinvoice_start = taxinvoice_start +1 WHERE invoicetype_id = ".$POST['invoicetype_id']);
				...
				//var_dump($info);
			$inserpoid=add_record('tbl_journal', $info, $dbcon);
				
				$info_trn['journal_id']				= $inserpoid;
				$info_trn['journal_trn_status']= 0;
			$updateid=update_record('tbl_journal_trn', $info_trn,"journal_trn_status=3 and user_id=".$_SESSION['user_id'] , $dbcon);
			
			insert_general_book($dbcon,$inserpoid);
			...
			echo json_encode($arr);					
				...
				$info['user_id']		= $_SESSION['user_id'];
				$info['company_id']		= $_SESSION['company_id'];
			$updateid=update_record('tbl_journal', $info,"journal_id=".$POST['journal_id'] , $dbcon);
			
				insert_general_book($dbcon,$POST['journal_id']);	
				if(isset($POST['save_print']))
				{
					$arr['printstatus']=$POST['print_status'];
					$arr['msg']="update";
			...
			$table='tbl_journal_trn';$tableid='journal_trn_id';
			...
			while($rel=mysqli_fetch_assoc($result))
			{
			 echo '<tr id="fieldtr'.$id.'" >
					...
					echo '<td data-label="Action" style="vertical-align:top">
							<button type="button" class="btn btn-round btn-warning btn-xs" onclick="edit_data('.$rel['journal_trn_id'].');" id="fieldr'.$i.'"><i class="fa fa-pencil"></i></button>
							<button type="button" class="btn btn-round btn-danger btn-xs" onclick="delete_data('.$rel['journal_trn_id'].');" id="fieldremove'.$i.'"><i class="fa fa-times"></i></button>
						</td>	
					</tr>';
					if($rel['entry_type']=="1"){
		...
		{
			$query="select * from tbl_invoicetype where status=0 and type_id=9 and company_id=".$_SESSION['company_id'];
			$result=$dbcon->query($query);
